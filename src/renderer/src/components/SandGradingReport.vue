...
`
    ...
    onMounted(() => {
      ...
      // 在 setup 函数中添加接收数据的逻辑
      window.electron.ipcRenderer.on('update-grading-data', (_, data) => {
        reportData.value = {
        ...
        // 重新计算细度模数
        fineModulus.value = (data.cumulative.reduce((sum, val) => sum + (100 - val), 100) / 100).toFixed(2);
    ...
    // 当组件卸载时，移除事件监听
    onUnmounted(() => {
      window.electron.ipcRenderer.removeAllListeners('update-grading-data');
    });
