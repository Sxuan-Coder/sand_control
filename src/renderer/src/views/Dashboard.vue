...
export default {
  ...
  setup() {
    ...
    const loadLocalImages = async () => {
      try {
        ...
        if (cachedImages.value.length === 0) {
          addLog(
            `未找到图片，使用测试图片代替。查找路径: ${globalImagesPath.value} 和 ${localImagesPath.value}`,
            'warning'
          )
        ...
        cachedImages.value = [
          {
            path: imgPath,
            actualPath: `${dataPath.value}/global/5_1.jpg (测试图片)`,
            group: 5,
            photo: 1,
            source: 'global',
            time: new Date().toLocaleTimeString()
          ...
          {
            path: imgPath,
            actualPath: `${dataPath.value}/local/5_1.jpg (测试图片)`,
            group: 5,
            photo: 1,
            source: 'local',
            time: new Date().toLocaleTimeString()
          ...
          {
            path: imgPath,
            actualPath: `${dataPath.value}/global/5_2.jpg (测试图片)`,
            group: 5,
            photo: 2,
            source: 'global',
            time: new Date().toLocaleTimeString()
          ...
          {
            path: imgPath,
            actualPath: `${dataPath.value}/local/5_2.jpg (测试图片)`,
            group: 5,
            photo: 2,
            source: 'local',
            time: new Date().toLocaleTimeString()
    ...
    const handleImageError = (e) => {
      ...
      if (index !== -1) {
        console.log('应该访问的实际路径:', latestPhotos.value[index].actualPath)
        latestPhotos.value[index].path = ''
        addLog(`图片加载失败: ${e.target.src}`, 'error')
      }
    }
    ...
    const handleImageLoad = (e) => {
      ...
      addLog(`图片加载成功: ${e.target.src}`, 'success')
    }
    ...
    const testUpdatePhotos = () => {
      ...
      // 使用测试图片
      const imgPath = '/test-image.jpg' // 使用绝对路径，不要加./
    ...
    const updateLatestPhotos = () => {
      try {
        ...
        const logMessage = selectedFiles
